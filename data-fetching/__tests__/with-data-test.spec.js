...
import withData from '../src/components/with-data'
import getJSON from '../src/components/get-json'
...
jest.mock('../src/components/get-json', () => jest.fn(() => ({ then: callback => callback('data') })))

test('passes the props to the component', () => {
  const ListWithGists = withData()(List)
  const username = 'gaearon'

  const wrapper = mount(<ListWithGists username={username} />)
